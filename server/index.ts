...
import { startConsumer } from "./kafka-consumer" // <--- Import the consumer
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("joinGame", ({ username }: { username: string }) => {
    ...
    // Prevent matching with yourself (e.g. two tabs)
  ...
  socket.on("disconnect", () => {
    ...
    // Handle active game disconnection (Forfeit)
    const gameId = playerToGame.get(socket.id)
    if (gameId) {
      const game = activeGames.get(gameId)
      if (game && !game.isBot) {
...
async function start() {
  try {
    ...
    // 2. Initialize Kafka Producer (To send events)
    await initKafka()
    
    // 3. Start Kafka Consumer (To read events and save to analytics DB)
    await startConsumer() 
